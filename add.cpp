...
int add (creator *& _creator , size_t * size , size_t * size_new)
{
    if ( *size_new >= *size) {
        ...
        for (  ; i < *size_new ; i++)
        {
            ...
            temp[i].set_surnameCre(randSurname());
            temp[i].set_nameCre(randName());
            temp[i].set_ageCre(randSize_t(20,60));
            temp[i].set_yearCre(randSize_t(1980,2022));
            temp[i].set_monthCre(randSize_t(1,12));
            temp[i].set_dayCre(randSize_t(1,28));
        }
        delete[] _creator;
        _creator = nullptr;
        _creator = temp;
